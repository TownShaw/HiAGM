...
class Trainer(object):
    ...
    def update_lr(self):
        ...
        logger.warning('Learning rate update {}--->{}'
                       .format(self.optimizer.param_groups[0]['lr'],
                               self.optimizer.param_groups[0]['lr'] * self.config.dict['train']['optimizer']['lr_decay']))
        for param in self.optimizer.param_groups:
            param['lr'] = self.config.dict['train']['optimizer']['learning_rate'] * self.config.dict['train']['optimizer']['lr_decay']
    ...
    def run(self, data_loader, epoch, stage, mode='TRAIN'):
        ...
        for batch in tqdm.tqdm(data_loader):
            ...
            if self.config.dict['train']['loss']['recursive_regularization']['flag']:
                recursive_constrained_params = self.model.hiagm.linear.weight
            else:
                recursive_constrained_params = None
            loss = self.criterion(logits,
                                  batch['label'].to(self.config.dict['train']['device_setting']['device']),
        ...
        if mode == 'EVAL':
            metrics = evaluate(predict_probs,
                               target_labels,
                               self.vocab,
                               self.config.dict['eval']['threshold'])
